...
import { parseItemId, getItemIconUrl } from '@/lib/albion/utils';
    ...
    <div className="flex flex-col gap-6 animate-in fade-in duration-500">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-[var(--mw-card)]/80 border border-[var(--mw-border)] rounded-xl p-5 shadow-lg relative overflow-hidden group">
          ...
          <p className="text-sm text-[var(--mw-text-muted)] uppercase tracking-widest font-bold mb-1">Total Analisado (Sessão)</p>
        ...
        <div className="bg-[var(--mw-card)]/80 border border-[var(--mw-border)] rounded-xl p-5 shadow-lg relative overflow-hidden group">
          ...
          <p className="text-sm text-[var(--mw-text-muted)] uppercase tracking-widest font-bold mb-1">Maior Lucro</p>
          <div className="text-3xl font-black text-[var(--mw-gold-bright)] font-mono">{formatSilver(stats.highestProfit)}</div>
          {isNewRecord && <span className="absolute top-2 right-2 px-2 py-0.5 bg-[var(--mw-gold-bright)]/20 border border-[var(--mw-gold-bright)]/50 text-[var(--mw-gold-bright)] text-sm uppercase font-bold rounded animate-pulse">Recorde Pessoal!</span>}
        </div>
        <div className="bg-[var(--mw-card)]/80 border border-[var(--mw-border)] rounded-xl p-5 shadow-lg relative overflow-hidden group">
          <div className="absolute -right-6 -bottom-6 opacity-5 group-hover:opacity-10 transition-opacity"><MapPin size={100} /></div>
          <p className="text-sm text-[var(--mw-text-muted)] uppercase tracking-widest font-bold mb-1">Melhor Cidade (Trend)</p>
        ...
        <div className="bg-[var(--mw-card)]/80 border border-[var(--mw-border)] rounded-xl p-5 shadow-lg relative overflow-hidden group">
          ...
          <p className="text-sm text-[var(--mw-text-muted)] uppercase tracking-widest font-bold mb-1">Categoria Quente</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-[var(--mw-card)]/80 border border-[var(--mw-border)] rounded-xl p-6 shadow-lg">
          <h3 className="font-bold text-[var(--mw-text-main)] uppercase tracking-widest text-sm mb-6 border-b border-[var(--mw-border)] pb-2 flex items-center gap-2"><Trophy size={16} /> Distribuição de Lucros</h3>
            ...
            {Object.entries(stats.profitDistribution).map(([label, value]) => {
               ...
               return (
                 <div key={label} className="flex items-center gap-3">
                   <div className="w-20 text-sm font-mono text-[var(--mw-text-muted)] text-right">{label}</div>
                   <div className="flex-1 h-6 bg-[var(--mw-bg)] rounded overflow-hidden relative border border-[var(--mw-border)]">
                     ...
                     />
                     <div className="absolute inset-y-0 left-2 flex items-center text-sm font-bold text-white drop-shadow-md">
                       {value > 0 ? value : ''}
                     </div>
                   </div>
                 </div>
        ...
        {/* Timeline */}
        <div className="bg-[var(--mw-card)]/80 border border-[var(--mw-border)] rounded-xl p-6 shadow-lg">
          <h3 className="font-bold text-[var(--mw-text-main)] uppercase tracking-widest text-sm mb-6 border-b border-[var(--mw-border)] pb-2 flex items-center gap-2"><Activity size={16} /> Timeline de Refreshes</h3>
               ...
               {refreshes.map((r, i) => (
                 <div key={r.timestamp} className="relative">
                   ...
                   <div className="flex items-center justify-between">
                     <span className="text-sm font-mono text-[var(--mw-text-muted)]">{new Date(r.timestamp).toLocaleTimeString('pt-BR')}</span>
                     <span className="font-bold text-[var(--mw-gold-primary)] text-sm">{r.found} ops</span>
                   </div>
                   <div className="text-sm text-[var(--mw-text-muted)] mt-1 tracking-widest uppercase">
      ...
      <div className="bg-[var(--mw-card)]/80 border border-[var(--mw-border)] rounded-xl p-6 shadow-lg mb-10">
          <h3 className="font-bold text-[var(--mw-text-main)] uppercase tracking-widest text-sm mb-6 border-b border-[var(--mw-border)] pb-2 flex items-center gap-2"><Crown size={16} className="text-[var(--mw-gold-bright)]" /> Ranking Histórico (Global 7 Dias)</h3>
          
          <div className="overflow-x-auto">
            <table className="w-full text-left">
              <thead>
                <tr className="border-b border-[var(--mw-border)] text-sm uppercase tracking-widest text-[var(--mw-text-muted)]">
              ...
              <tbody className="text-sm">
                {loading ? (
                  ...
                  const iconUrl = getItemIconUrl(flip.itemId, 1, 32);
                  return (
                    <tr key={flip.id} className="border-b border-[var(--mw-border)]/30 hover:bg-[var(--mw-bg)]/50 transition-colors">
                      ...
                      <td className="py-3 px-2 flex items-center gap-2">
                         ...
                         <span className="truncate max-w-[200px] text-sm font-semibold text-[var(--mw-text-muted)]" title={flip.itemName}>{flip.itemName}</span>
                      </td>
