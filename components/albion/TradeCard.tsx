...
import { TradeCardTrend } from './trade/TradeCardTrend';
      ...
      {isHotDeal && (
         <div className="absolute top-0 right-0 z-10 translate-x-1.5 -translate-y-1.5">
           <span className="flex h-5 items-center gap-1">
             ...
             <span className="relative inline-flex rounded-full bg-gradient-to-r from-amber-500 to-orange-500 text-sm font-black text-slate-950 px-2 py-0.5 shadow-lg border border-amber-300/50 uppercase tracking-widest whitespace-nowrap">
               🔥 Hot Deal
             </span>
           </span>
         </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2">
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-4 gap-4">
                <h4 className="text-sm font-black text-[var(--mw-text-muted)] flex items-center gap-2 uppercase tracking-widest bg-[var(--mw-card)] border border-[var(--mw-border)] px-3 py-1.5 rounded w-fit m-0">
                  <Route size={14} className="text-[var(--mw-gold-primary)]" /> Roteiro de Execução
                </h4>
                
                <div className="flex items-center gap-2 bg-[var(--mw-card)] border border-[var(--mw-border)] px-3 py-1.5 rounded text-sm shadow-inner">
              ...
              <div className="bg-[var(--mw-card)]/40 p-2 sm:p-4 rounded-xl border border-[var(--mw-border)]">
                 ...
                 <div className="mt-6 p-4 bg-gradient-to-r from-[var(--mw-gold-primary)]/10 to-transparent border-l-4 border-[var(--mw-gold-primary)] rounded-r-lg flex flex-col sm:flex-row justify-between items-center gap-4 shadow-sm">
                   <div className="flex items-center gap-3">
                     ...
                     <div>
                       <span className="text-[var(--mw-text-main)] font-bold uppercase tracking-wide text-sm block">Balanço Final Esperado</span>
                       <span className="text-sm text-[var(--mw-text-muted)]">Custo Total: {formatSilver((result.buyPrice + (result.tradeType === 'enchant' && result.baseCost ? result.baseCost : 0)) * quantity)}</span>
                     </div>
                   </div>
                   <div className="text-right">
                     ...
                     <div className="text-sm text-[var(--mw-green)] font-mono bg-[var(--mw-green)]/10 px-2 py-0.5 rounded border border-[var(--mw-green)]/20 inline-block mt-1">Margem Real: {result.margin.toFixed(1)}%</div>
                   </div>
                 </div>
              </div>
            </div>
            ...
            <div className="flex flex-col gap-6">
              ...
              <TradeCardTrend itemId={result.itemId} />
            </div>
          </div>
        </div>
      )}
