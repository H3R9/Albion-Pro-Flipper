...
export function TradeCardHeader({ result }: TradeCardHeaderProps) {
  ...
  const iconUrl = getItemIconUrl(result.itemId, result.quality, 56);
  ...
  return (
    <div className="flex items-center gap-4 lg:w-[35%] shrink-0">
      <div className="relative w-14 h-14 bg-gradient-to-br from-slate-800 to-slate-950 rounded-lg flex items-center justify-center border-2 shadow-inner" style={{ borderColor: qi.color }}>
        ...
        <span className="absolute -bottom-2 -right-2 text-sm font-black px-1.5 py-0.5 rounded shadow-md bg-[var(--mw-bg)] border border-[var(--mw-border)] text-[var(--mw-text-main)]">
          T{p.tier}.{p.enchant}
        </span>
        
        {result.flipScore && (
          <div className="absolute -top-2 -left-2 w-7 h-7 rounded-full flex items-center justify-center shadow-lg border-2 z-10 font-bold text-sm cursor-help" 
      ...
      <div className="flex flex-col min-w-0">
        ...
        <div className="flex flex-wrap gap-2 text-sm text-[var(--mw-text-muted)] mt-1.5 items-center">
          ...
          {result.volume24h !== undefined && (
            <span className={cn("px-1.5 py-0.5 rounded text-sm font-bold flex items-center border shadow-sm", 
        ...
        <div className="flex mt-2 items-center gap-1 cursor-help" title={overallAgeIndicator.tooltip}>
           <span className="text-sm text-[var(--mw-text-muted)] uppercase font-bold tracking-wider mr-1">Idade:</span>
           <div className={cn("w-2 h-2 rounded-full", overallAgeIndicator.bg)}></div>
           <span className={cn("text-sm font-bold uppercase", overallAgeIndicator.text)}>{overallAgeIndicator.label}</span>
        </div>
      </div>
    </div>
  );
