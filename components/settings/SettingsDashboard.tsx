...
import { Settings, FileText, BellRing, Save, ChevronRight, ShieldAlert, BookOpen, MapPin, Webhook } from 'lucide-react';
              ...
              <div className="bg-[#0f0f11] backdrop-blur-md border border-white/5 rounded-lg shadow-2xl p-8 space-y-10 relative overflow-hidden ring-1 ring-white/5">
                ...
                <div className="relative">
                  <div className="flex items-center justify-between mb-8 pb-4 border-b border-white/5">
                    ...
                    <div className="px-3 py-1 bg-green-500/10 text-green-400 border border-green-500/20 text-sm font-bold uppercase tracking-widest rounded-sm">
                      Scanner Ativo
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-black/20 p-6 rounded-lg border border-white/5">
                    <div className="space-y-4 group">
                      <label className="block text-sm font-black text-amber-500/80 uppercase tracking-widest">Gatilho de Ouro (Prata Base)</label>
                      ...
                      <p className="text-sm text-[var(--mw-text-muted)] tracking-wide leading-relaxed">
                        Filtro de Ruído: Ignorar trades com lucro absoluto. Ex: <span className="text-white">100.000</span>
                      </p>
                    </div>

                    <div className="space-y-4 group">
                      <label className="block text-sm font-black text-amber-500/80 uppercase tracking-widest">Margem de Esforço (%)</label>
                      <div className="relative group">
                        <div className="absolute inset-y-0 left-0 flex items-center pl-4 text-amber-500/50 font-mono pointer-events-none transition-colors group-focus-within:text-amber-400 font-bold">%</div>
                        <input 
                          type="number"
                  ...
                  <div className="mt-8 space-y-4 group bg-black/20 p-6 rounded-lg border border-[#5865F2]/30 shadow-[0_0_15px_rgba(88,101,242,0.05)]">
                    <label className="flex items-center gap-2 text-sm font-black text-[#5865F2] uppercase tracking-widest">
                      <Webhook size={14} /> Integração com Webhook (Discord/Telegram)
                    </label>
                    <div className="relative group">
                      <div className="absolute inset-y-0 left-0 flex items-center pl-4 text-white/30 font-mono pointer-events-none transition-colors group-focus-within:text-[#5865F2]">🔗</div>
                      <input 
                        type="password"
                        placeholder="https://discord.com/api/webhooks/..."
                        className="w-full bg-[#151518] border border-white/10 rounded-md h-14 pl-12 pr-4 text-white outline-none focus:border-[#5865F2] focus:bg-[#1a1a1e] focus:shadow-[0_0_15px_rgba(88,101,242,0.1)] transition-all font-mono text-sm shadow-inner placeholder-white/20"
                        value={alertSettings?.webhookUrl || ''}
                        onChange={e => setAlertSettings({ ...alertSettings, webhookUrl: e.target.value })}
                      />
                    </div>
                    <p className="text-sm text-[var(--mw-text-muted)] tracking-wide leading-relaxed">
                      Deixe a aba aberta rodando o Auto-Refresh que o Scanner enviará os Alertas direto para o seu servidor Discord/Telegram usando a URL do Webhook.
                    </p>
                  </div>
                </div>
                ...
                <div className="relative bg-black/20 p-6 rounded-lg border border-white/5">
                  ...
                  <div className="flex flex-wrap gap-3">
                    <motion.button
                      ...
                      className={cn("px-6 py-3 rounded-md text-sm font-black uppercase tracking-widest transition-all duration-300 flex items-center gap-2", 
                    ...
                    {CITIES.filter(c => c !== 'Caerleon').map(city => {
                      ...
                      return (
                        <motion.button
                          ...
                          className={cn("px-4 py-3 rounded-md text-sm font-bold uppercase tracking-widest transition-all duration-300 flex items-center justify-center min-w-[100px] disabled:opacity-20 disabled:cursor-not-allowed", 
                            isActive 
