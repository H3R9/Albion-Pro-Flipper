  ...
  return (
    <div className="space-y-1.5">
      {label && (
        <label
          htmlFor={inputId}
          className="block text-sm font-bold uppercase tracking-widest text-[var(--mw-text-muted)]"
        >
          {label}
        </label>
      )}
      ...
      {hint && !error && (
        <p className="text-sm text-[var(--mw-text-muted)] leading-relaxed">{hint}</p>
      )}
      {error && (
        <p className="text-sm text-[var(--mw-red)] font-bold">{error}</p>
      )}
    </div>
  );
}
