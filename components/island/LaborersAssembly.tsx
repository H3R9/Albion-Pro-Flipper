...
export function LaborersAssembly({ onItemClick }: { onItemClick?: (id: string) => void }) {
  ...
  const { specificTrophyTiers, generalTrophyTiers, extraTrophies, maxTrophyHappiness } = useMemo(() => {
    let spec: number[] = [];
    let gen: number[] = [];
    let ext: string[] = [];
    let maxHapp = 0;
    
    if (laborer.hasSpecificTrophy) {
      const minT = laborer.trophyMinTier || 2;
      for (let t = minT; t <= targetTier; t++) {
        spec.push(t);
        maxHapp += 10;
      }
      
      const slotsLeft = 10 - spec.length;
      for (let t = targetTier; t >= 2 && gen.length < slotsLeft; t--) {
        gen.push(t);
        maxHapp += 5;
      }
    } else {
      ...
      if (targetTier >= 3) {
        ext.push('UNIQUE_FURNITUREITEM_FOUNDER_LOOKINGGLASS');
        maxHapp += 5;
      }
      if (targetTier === 8) {
        ext.push('T8_FURNITUREITEM_TROPHY_FISHING_BOSS');
        maxHapp += 5;
      }
    }
    
    // Sort reverse so higher tiers appear first
    gen.sort((a,b) => b - a);
    spec.sort((a,b) => b - a);

    return { specificTrophyTiers: spec, generalTrophyTiers: gen, extraTrophies: ext, maxTrophyHappiness: maxHapp };
  }, [laborer, targetTier]);
    ...
    <div className="space-y-6">
      <div className="bg-[var(--mw-card)] rounded-xl border border-[var(--mw-border)] p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          <div>
            <label className="text-sm font-bold uppercase tracking-widest text-[var(--mw-text-muted)] mb-2 block">Tipo de Construção</label>
          ...
          <div>
            <label className="text-sm font-bold uppercase tracking-widest text-[var(--mw-text-muted)] mb-2 block">Tier da Construção</label>
            <select 
          ...
          <div>
            <label className="text-sm font-bold uppercase tracking-widest text-[var(--mw-text-muted)] mb-2 block">Quantidade</label>
            <input 
              type="number"
              min={1}
              value={quantity}
          ...
          <div className="flex flex-col justify-end">
            <div className="w-full rounded-xl bg-emerald-500/10 flex items-center p-2.5 border border-emerald-500/30 gap-3">
              ...
              <div>
                <div className="text-sm font-bold uppercase tracking-widest text-emerald-500/80">Capacidade Total</div>
        ...
        <div className="flex flex-wrap gap-2">
          {LABORERS.map(l => {
            ...
            return (
              <button 
                key={l.id} 
                className={`flex-1 sm:flex-none px-3 py-1.5 rounded-lg text-sm font-bold uppercase transition-colors border ${
                  isSelected 
                    ? 'bg-emerald-500/20 border-emerald-500/50 text-emerald-400' 
                    : 'bg-[var(--mw-bg)] border-[var(--mw-border)] text-[var(--mw-text-muted)] hover:text-[var(--mw-text-main)]'
                }`}
      ...
      <div className="bg-[var(--mw-card)] rounded-xl border border-[var(--mw-border)] overflow-hidden">
        <div className="p-6 border-b border-[var(--mw-border)] bg-[var(--mw-bg)]/50 relative">
          {isCalculating && (
            <div className="absolute top-4 right-4 flex items-center gap-2 text-emerald-500 text-sm font-bold uppercase">
        ...
        <div className="p-6 grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="space-y-4">
            ...
            <div className="text-sm text-[var(--mw-text-muted)] italic leading-tight">
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-3">
              <div>
                <label className="text-sm font-bold uppercase text-[var(--mw-text-muted)] mb-1 block">Retorno Estimado (Prata Bruta vs Recursos)</label>
          ...
          <div className="flex flex-col justify-center items-center bg-emerald-500/5 rounded-xl border border-emerald-500/20 p-6 relative overflow-hidden">
            ...
            <div className="text-sm font-bold uppercase tracking-widest text-[var(--mw-text-muted)] mb-1">
            ...
            <div className="text-sm text-[var(--mw-text-muted)] mb-6 text-center">
          ...
          <div className="p-6">
            <div className="space-y-8">
              ...
              <div className="bg-sky-500/10 border border-sky-500/30 rounded-lg p-4 text-sm text-[var(--mw-text-main)]">
                ...
                <p className="text-sm text-[var(--mw-text-muted)] mt-2">
                ...
                <ul className="list-disc pl-5 mt-3 space-y-1 text-sm text-[var(--mw-text-muted)]">
    ...
    >
      ...
      <div className="flex-grow min-w-0">
        <div className="font-mono text-sm font-bold text-[var(--mw-text-main)] mb-0.5 flex justify-between">
          ...
          {unitPrice !== undefined && (
            <span className="text-amber-400 text-sm">{formatSilver(unitPrice * amount)}</span>
          )}
        </div>
        <div className="text-sm font-bold text-[var(--mw-text-main)] truncate" title={name}>
          {name}
        </div>
        {note && <div className="text-sm text-[var(--mw-text-muted)] mt-1">{note}</div>}
      </div>
    </div>
  );
}
