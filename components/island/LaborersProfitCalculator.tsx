...
// Yields at 100% (multiply by 1.5 for 150% yield)
const GATHER_BASE: Record<number, number> = { 4: 32, 5: 21.33, 6: 16, 7: 10.66, 8: 8 };
const CRAFT_BASE_100: Record<number, number> = { 4: 16, 5: 8, 6: 5.3, 7: 4.46, 8: 4.1 };
const MERC_BASE: Record<number, number> = { 4: 1123, 5: 1746, 6: 2690, 7: 4202, 8: 6584 };
...
export function LaborersProfitCalculator() {
  ...
  const allNeededItems = useMemo(() => {
    ...
    for (const t of TIERS) {
      ...
      items.add(`T${t}_FISH_FRESHWATER_AVALON_RARE`);
    ...
    return (
      <div key={worker.id} className="bg-black/30 border border-white/5 rounded-lg overflow-hidden flex flex-col hover:border-white/10 transition-colors">
        <div className="flex items-center gap-3 p-4 border-b border-white/5 bg-black/40">
           <Image src={getItemIconUrl(worker.img)} alt={worker.en} width={40} height={40} className="object-contain drop-shadow-md" />
           <div>
             <h3 className="font-bold text-sm text-[var(--mw-text-main)] uppercase tracking-wide">{worker.pt}</h3>
             <p className="text-sm text-[var(--mw-text-muted)] uppercase tracking-widest">{worker.mat}</p>
           </div>
        </div>
        <div className="grid grid-cols-5 text-center divide-x divide-white/5">
          {TIERS.map(t => {
            ...
            const exactYield = baseYield * (happiness / 100);
            const actualYield = Math.floor(exactYield); // for display
            ...
            const revenue = exactYield * expectedResourcePrice;
            ...
            return (
              <div key={t} className="p-3 flex flex-col items-center justify-center gap-1">
                <span className="text-sm font-bold text-white/40">T{t}</span>
                <span className={cn("text-sm font-black", profit >= 0 ? "text-emerald-400 drop-shadow-[0_0_8px_rgba(52,211,153,0.3)]" : "text-red-400")}>
                  {formatSilver(profit)}
                </span>
                <span className="text-sm text-white/30 truncate w-full" title={`Qtd: ${actualYield} | Preço Médio: ${formatSilver(expectedResourcePrice)}`}>
                  {actualYield} rec.
                </span>
              </div>
            );
    ...
    return (
      <div key={worker.id} className="bg-black/30 border border-white/5 rounded-lg overflow-hidden flex flex-col hover:border-white/10 transition-colors">
        <div className="flex items-center gap-3 p-4 border-b border-white/5 bg-black/40">
           ...
           <div>
             <h3 className="font-bold text-sm text-[var(--mw-text-main)] uppercase tracking-wide">{worker.pt}</h3>
             <p className="text-sm text-[var(--mw-text-muted)] uppercase tracking-widest truncate w-40" title={worker.mat}>{worker.mat}</p>
           </div>
        </div>
        <div className="grid grid-cols-5 text-center divide-x divide-white/5">
          {TIERS.map(t => {
            const exactQtyTotal = (CRAFT_BASE_100[t as keyof typeof CRAFT_BASE_100] || 0) * (happiness / 100);
            const qtyTotal = Math.floor(exactQtyTotal);
            let revenue = 0;
            
            for (const [item, pct] of Object.entries(worker.mix)) {
               const exactQty = exactQtyTotal * (pct as number);
               revenue += exactQty * getP(`T${t}_${item}`);
            }
            ...
            return (
              <div key={t} className="p-3 flex flex-col items-center justify-center gap-1">
                <span className="text-sm font-bold text-white/40">T{t}</span>
                <span className={cn("text-sm font-black", profit >= 0 ? "text-sky-400 drop-shadow-[0_0_8px_rgba(56,189,248,0.3)]" : "text-red-400")}>
                  {formatSilver(profit)}
                </span>
                <span className="text-sm text-white/30 truncate w-full" title={`Qtd: ${qtyTotal}`}>
                  {qtyTotal} ref.
                </span>
              </div>
            );
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="md:col-span-2 bg-[#151518] rounded-xl border border-white/5 p-6 shadow-xl relative overflow-hidden">
          ...
          <div className="flex flex-col sm:flex-row gap-6 relative">
            <div className="flex-1 space-y-2">
              <label className="text-sm font-black tracking-widest uppercase text-white/40 flex items-center gap-2">
              ...
              <p className="text-sm text-white/30 leading-relaxed max-w-sm">Nota: Valores de retorno são estimativas baseadas em dados da comunidade. O retorno real depende do tier do diário vs tier do trabalhador. Usar diário 1 tier abaixo pode resultar em até 150% de retorno.</p>
            </div>

            <div className="w-px bg-white/5 hidden sm:block"></div>

            <div className="space-y-2 flex-1">
               <label className="text-sm font-black tracking-widest uppercase text-white/40">Estratégia de Diário</label>
               <div className="flex bg-black/40 rounded-lg p-1 border border-white/5">
                 <button 
                  onClick={() => setFillMode('buy_full')}
                  className={cn("flex-1 py-2 text-sm font-bold rounded-md uppercase tracking-wider transition-all", 
            ...
            <div className="space-y-2 flex-1">
               <label className="text-sm font-black tracking-widest uppercase text-white/40">Cidade BDF</label>
               <select 
                  value={city} 
        ...
        <div className="bg-gradient-to-br from-emerald-900/20 to-sky-900/20 rounded-xl border border-white/5 p-6 shadow-xl flex flex-col justify-center relative overflow-hidden">
           ...
           <p className="text-sm text-white/50 mb-4 mt-1 leading-relaxed">
           ...
           <div className="flex gap-2 text-sm text-white/40 tracking-wider">
             ...
             <div className="bg-black/30 border border-white/5 rounded-lg overflow-hidden flex flex-col hover:border-white/10 transition-colors">
                <div className="flex items-center gap-3 p-4 border-b border-white/5 bg-black/40">
                   ...
                   <div>
                     ...
                     <p className="text-sm text-[var(--mw-text-muted)] uppercase tracking-widest">Prata Fixa (acima de 100%: chance de duplicar)</p>
                   </div>
                </div>
                <div className="grid grid-cols-5 text-center divide-x divide-white/5">
                  {TIERS.map(t => {
                    const base = MERC_BASE[t as keyof typeof MERC_BASE];
                    // Above 100% happiness, the extra % is a CHANCE TO DOUBLE the reward.
                    // At 150%, there's a 50% chance to double, so expected value = base * 1.5
                    // The formula base * (happiness/100) gives the same numerical result.
                    const exactSilver = base * (happiness / 100);
                    const avgSilver = Math.round(exactSilver);
                    
                    const emptyCost = getP(`T${t}_JOURNAL_MERCENARY_EMPTY`) || JOURNAL_NPC_PRICE[t];
                    const fullCost = getP(`T${t}_JOURNAL_MERCENARY_FULL`);
                    
                    const cost = fillMode === 'buy_full' ? fullCost : emptyCost;
                    const profit = exactSilver - cost + (fillMode === 'buy_full' ? emptyCost : 0);

                    return (
                      <div key={t} className="p-3 flex flex-col items-center justify-center gap-1">
                        <span className="text-sm font-bold text-white/40">T{t}</span>
                        <span className={cn("text-sm font-black", profit >= 0 ? "text-rose-400 drop-shadow-[0_0_8px_rgba(244,63,94,0.3)]" : "text-red-400")}>
                          {formatSilver(profit)}
                        </span>
                        <span className="text-sm text-white/30 truncate w-full" title={`Média de Prata: ${avgSilver}`}>
                          {avgSilver} Prata
                        </span>
                      </div>
                    );
              ...
              <div className="bg-black/30 border border-white/5 rounded-lg overflow-hidden flex flex-col hover:border-white/10 transition-colors mt-2">
                <div className="flex items-center gap-3 p-4 border-b border-white/5 bg-black/40">
                   ...
                   <div>
                     ...
                     <p className="text-sm text-[var(--mw-text-muted)] uppercase tracking-widest">Base Px. Comum (Drop Variável)</p>
                   </div>
                </div>
                <div className="p-3 bg-blue-950/20 border-b border-blue-500/10 text-center">
                  <p className="text-sm text-blue-300/60 leading-relaxed font-medium">Atenção: Pescadores requerem <strong>Peixes Empalhados (Ex: Tubarão)</strong> na casa para 150%.<br/>Calculando o retorno como uma cesta estimada: <strong className="text-blue-300">80% Peixe Comum + 10% Peixes Raros (Enguias, etc) + 10% Algas</strong>.</p>
                </div>
                <div className="grid grid-cols-5 text-center divide-x divide-white/5">
                  {TIERS.map(t => {
                    const baseYield = GATHER_BASE[t as keyof typeof GATHER_BASE];
                    const exactYield = baseYield * (happiness / 100);
                    const actualYield = Math.floor(exactYield);
                    ...
                    const pRares = [
                      ...
                      getP(`T${t}_FISH_FRESHWATER_AVALON_RARE`)
                    ].filter(p => p > 0);
                    ...
                    const revenue = exactYield * estimatedItemPrice;
                    ...
                    return (
                      <div key={t} className="p-3 flex flex-col items-center justify-center gap-1 hover:bg-white/5 transition-colors">
                        ...
                        <span className={cn("text-sm font-black", 
                          profit >= 0 ? "text-blue-400 drop-shadow-[0_0_8px_rgba(96,165,250,0.3)]" : "text-red-400"
                        )}>
                          ~{formatSilver(profit)}
                        </span>
                        <span className="text-sm text-white/30 truncate w-full" title={`Base estimada cesta (80% com., 10% raro, 10% alga) x ${actualYield} itens`}>
                          {actualYield} base
                        </span>
                      </div>
                    );
