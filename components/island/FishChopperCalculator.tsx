...
export function FishChopperCalculator() {
  ...
  const allNeededItems = useMemo(() => {
    ...
    TIERS.forEach(t => {
      ...
      list.add(`T${t}_FISH_FRESHWATER_AVALON_RARE`);
    });
    ...
    <div className="flex flex-col gap-6 relative">
      ...
      <div className="bg-[var(--mw-card)] rounded-xl border border-[var(--mw-border)] p-4 flex flex-col md:flex-row items-center justify-between gap-4">
        <div className="flex items-center gap-3 w-full md:w-auto">
          <h3 className="text-sm font-bold uppercase tracking-widest text-[var(--mw-text-muted)] whitespace-nowrap">Cidade Onde Vai Vender:</h3>
          <select 
            value={selectedCity} 
        ...
        {isLoading ? (
          ...
          <div className="flex items-center gap-4 bg-cyan-950/30 border border-cyan-500/20 px-4 py-2 rounded-lg cursor-pointer hover:bg-cyan-900/40 transition-colors"
             ...
             <div>
               <p className="text-sm text-cyan-400/70 font-bold uppercase tracking-wider">Peixe Picado ({selectedCity})</p>
               <div className="flex items-center gap-2">
                 <p className="font-mono text-base font-black text-cyan-400">{formatSilver(choppedPrice)}</p>
                 {choppedPrice > 0 && <span className="text-sm text-white/30">{Math.round(choppedAge)}m atrás</span>}
               </div>
             </div>
          </div>
        )}
                     ...
                     <div key={itemId} className={cn("bg-black/30 border rounded-xl overflow-hidden flex items-stretch transition-colors",
                        ...
                        <div className="p-3 flex-1 flex flex-col justify-center">
                           ...
                           <div className="flex items-center gap-4">
                              <div>
                                 <p className="text-sm text-white/40 uppercase">Preço Venda</p>
                              ...
                              <div>
                                 <p className="text-sm text-white/40 uppercase">Rende {yieldAmount}x Picado</p>
                        ...
                        <div className={cn("p-3 w-28 flex flex-col items-center justify-center border-l shrink-0",
                           ...
                           {noData || choppedPrice === 0 ? (
                              <p className="text-sm font-bold text-white/30 uppercase">Sem Dados</p>
                           ) : (
                              <>
                                <p className="text-sm text-white/50 uppercase mb-1 font-bold text-center">
