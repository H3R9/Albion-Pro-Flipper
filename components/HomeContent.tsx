...
import { LayoutDashboard, ShoppingCart, Crosshair, PackageOpen, FileText, Calculator, Home, Hammer, Settings, Brain } from 'lucide-react';
...
const LootAnalyzer = lazy(() => import('@/components/albion/LootAnalyzer').then(m => ({ default: m.LootAnalyzer })));
...
const RefiningDashboard = lazy(() => import('@/components/refining/RefiningDashboard').then(m => ({ default: m.RefiningDashboard })));
const RefiningCalculator = lazy(() => import('@/components/refining/RefiningCalculator').then(m => ({ default: m.RefiningCalculator })));
...
const LazyFallback = () => (
  <div className="flex items-center justify-center py-20">
    <div className="flex flex-col items-center gap-3">
      ...
      <span className="text-sm text-[var(--mw-text-muted)] uppercase tracking-widest font-bold">Carregando módulo...</span>
    </div>
  </div>
);

export default function HomeContent() {
  const [appMode, setAppMode] = useState<'hub' | 'operations' | 'island' | 'settings' | 'refining'>('hub');
  
  const {
    isScanning, progress, results, filteredResults,
    currentTab, setCurrentTab, scanMode, setScanMode,
  ...
  const opsNavItems = [
    ...
    { id: 'loot-ai', label: 'Analista de Loot', icon: <Brain size={18} /> },
  ...
  const refiningNavItems = [
    { id: 'refining-guide', label: 'Guia de Refino', icon: <FileText size={18} /> },
    { id: 'refining-calculator', label: 'Calculadora de Refino', icon: <Calculator size={18} /> },
  ] as const;

  const handleSelectModule = (module: 'operations' | 'island' | 'settings' | 'refining') => {
    setAppMode(module);
    if (module === 'island') setCurrentTab('island-dashboard' as ScanTab);
    if (module === 'refining') setCurrentTab('refining-calculator' as ScanTab);
    if (module === 'operations' && ['island-dashboard', 'refining-guide', 'refining-calculator'].includes(currentTab as string)) setCurrentTab('dashboard');
  };

  const activeNavItems = appMode === 'island' ? islandNavItems : (appMode === 'refining' ? refiningNavItems : (appMode === 'settings' ? [] : opsNavItems));
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-[var(--mw-bg)] font-sans">
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden relative">
        ...
        <main className="flex-1 overflow-y-auto p-4 md:p-8 flex flex-col items-center scrollbar-hide w-full break-words">
          <div className="max-w-6xl mx-auto w-full h-full relative">
            
            {/* Settings Module */}
            {appMode === 'settings' && (
              <div className="w-full h-full">
                <button onClick={() => setAppMode('hub')} className="flex items-center gap-2 text-sm font-bold uppercase tracking-wider text-[var(--mw-text-muted)] hover:text-[var(--mw-text-main)] transition-colors mb-4">
                  ← Voltar à Arena
                </button>
                <ErrorBoundary>
            ...
            {appMode === 'refining' && (
              <div className="w-full h-full">
                ...
                <ErrorBoundary>
                  <Suspense fallback={<LazyFallback />}>
                    {currentTab === 'refining-guide' ? <RefiningDashboard /> : currentTab === 'refining-calculator' ? <RefiningCalculator /> : null}
                  </Suspense>
                </ErrorBoundary>
              </div>
            )}
            ...
            {appMode === 'operations' && (
              <>
                ...
                <div className={cn(currentTab !== 'loot-ai' && "hidden")}>
                  <ErrorBoundary>
                    <Suspense fallback={<LazyFallback />}>
                      <LootAnalyzer />
                    </Suspense>
