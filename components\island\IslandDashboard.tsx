        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div className="col-span-1 lg:col-span-4 flex flex-col gap-4">
            <div className="bg-[var(--mw-card)] rounded-xl border border-[var(--mw-border)] p-4">
              <h3 className="text-xs font-bold uppercase tracking-widest text-[var(--mw-text-muted)] mb-3">Categoria</h3>
              ...
              <h3 className="text-xs font-bold uppercase tracking-widest text-[var(--mw-text-muted)] mt-6 mb-3">Construção</h3>
              <div className="flex flex-col gap-2 max-h-[300px] overflow-y-auto scrollbar-hide">
                {BUILDINGS.filter(b => b.category === selectedCategory).map(b => (
                  ...
                  >
                    ...
                    <span className="text-[10px] uppercase font-black opacity-50 border px-1.5 rounded">T{b.maxTier}</span>
                  </button>
                ))}
              </div>
            </div>
            ...
            {selectedBuilding && (
              <div className="bg-[var(--mw-card)] border border-[var(--mw-border)] rounded-xl p-4 flex flex-col gap-4">
                 <div>
                   <label className="text-xs font-bold uppercase tracking-widest text-[var(--mw-text-muted)] mb-2 block">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-4">
                  {materialIds.map(mat => (
                    <div key={mat} onClick={() => setPricesModalItem(mat)} className="bg-[var(--mw-bg)] border border-[var(--mw-border)] rounded-lg p-3 flex items-center gap-3 relative overflow-hidden group cursor-pointer hover:border-emerald-500/50 transition-colors">
                      ...
                      <div>
                        <p className="text-[10px] text-[var(--mw-text-muted)] leading-tight mb-0.5 line-clamp-1 truncate pr-2" title={getItemFullName(mat)}>
                          {getItemFullName(mat)}
                        </p>
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center gap-3">
                           <span className="w-6 h-6 rounded-full bg-[var(--mw-card)] flex items-center justify-center text-[10px] font-bold text-[var(--mw-text-muted)] border border-[var(--mw-border)]">
                             {index + 1}
                           </span>
                        ...
                        <div className="text-right">
                           {cost > 0 ? (
                             <div className="flex flex-col items-end">
                               ...
                               <span className="text-[10px] text-[var(--mw-text-muted)] uppercase tracking-widest font-bold">
                                 Atualizado há {Math.round(cityCosts[city]?.maxAge || 0)} min
                               </span>
                             </div>
                           ) : (
                             <span className="text-[10px] uppercase font-bold text-[var(--mw-text-muted)] px-2 py-1 bg-[var(--mw-card)] rounded border border-[var(--mw-border)]">
                               Sem Custo
                             </span>
                           )}
                        </div>
