...
export function getItemIconUrl(itemId: string, quality: number = 1, size: number = 64) {
  // Clean up itemId in case it incorrectly includes quality (e.g. T6_BAG@4@2)
  const cleanId = itemId.split('@').slice(0, 2).join('@');
  return `${RENDER_BASE}/${cleanId}.png?size=${size}&quality=${quality}`;
}
