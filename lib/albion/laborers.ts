...
export function getLaborerItemName(tier: number, itemId: string, workerName: string): string {
  // It turns out they ARE in item-names.json
  const exactName = getItemDisplayName(itemId);
  if (exactName !== itemId) return exactName;
  
  // Fallback to manual naming if not found for some reason
  ...
  if (itemId === 'UNIQUE_FURNITUREITEM_FOUNDER_LOOKINGGLASS') return 'Luneta do Explorador (T3)';
