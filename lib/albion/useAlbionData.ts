...
import { ROYAL_CITIES, parseItemId } from './utils';
import { scoreFlip } from './scoring';
    ...
    async (
      ...
      try {
        ...
        if (allResults.length > 0) {
          ...
          // Check if user set a minimum volume to filter bad items
          if (settings.minVolume && settings.minVolume > 0) {
             allResults = allResults.filter(r => (r.volume24h || 0) >= settings.minVolume!);
          }
